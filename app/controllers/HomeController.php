    ...
    { 
        ...
        try { 
            ...
            if ($produkKeranjang) { 
                ...
                if ($produk['stock'] < ($produkKeranjang['jumlah'] + 
$data['jumlah'])) { 
                    throw new Exception('Stok tidak cukup untuk menambah jumlah 
produk'); 
                } 
 
                $detailKeranjang['jumlah'] += $produkKeranjang['jumlah']; // Add the new quantity 
                $success = $this->detailKeranjang->edit($produkKeranjang['id'], 
$detailKeranjang); 
            } else { 
    ...
    { 
        ...
        foreach ($data['pesanan'] as $pesanan) { 
            ...
            if ($produk['stock'] < $pesanan['qty']) { 
                return ['icon' => 'error', 'message' => "Stok tidak mencukupi 
untuk beberapa produk dipilih"]; 
            } 
        } 
        ...
        if (!$success) { 
            ...
            return ['icon' => 'error', 'message' => 'Terjadi kesalahan saat 
menambahkan data']; 
        } else { 
            //kosongkan keranjang 
