...
{ 
    ...
    protected $fillable = ['nama', 'harga', 'stock', 'deskripsi', 'photo', 
'kategori_id']; 
    ...
    { 
        try { 
            $stmt = $this->db->prepare("SELECT p.*, k.nama as nama_kategori FROM 
$this->table as p JOIN kategori k ON p.kategori_id = k.id ORDER BY p.id DESC 
LIMIT 6"); 
            $stmt->execute(); 
            return $stmt->fetchAll(PDO::FETCH_ASSOC); 
        } catch (PDOException $e) { 
            throw new Exception("Error fetching all data from $this->table: " . 
$e->getMessage()); 
        } 
    } 
    ...
    { 
        try { 
            ...
            $stmt = $this->db->prepare("SELECT p.*, k.nama as nama_kategori FROM 
$this->table as p JOIN kategori k ON p.kategori_id = k.id WHERE k.nama LIKE 
:kategori"); 
...
$e->getMessage()); 
    ...
    { 
        ...
        try { 
            $stmt = $this->db->prepare("SELECT p.*, k.nama as nama_kategori FROM 
$this->table as p JOIN kategori k ON p.kategori_id = k.id WHERE p.nama LIKE 
:keyword"); 
