...
id="kategori" name="kategori"> 
            <option value="semua" <?= ($selectedCategory === 'semua') ? 
'selected' : '' ?>>Semua</option> 
            <?php foreach ($cKategori->semuaKategori() as $kategori) { ?> 
                <option value="<?= $kategori['nama'] ?>" <?= ($selectedCategory 
...
echo '<td class="text-center">Rp ' . 
number_format($produk['harga'], 0, ',', '.') . '</td>'; 
echo '<td class="text-center">' . 
htmlspecialchars($produk['stock']) . '</td>'; 
echo '<td class="text-center">' . 
htmlspecialchars($produk['nama_kategori']) . '</td>'; 
echo '<td class="text-center"><img src="' . $produk['photo'] . '" 
alt="foto ' . htmlspecialchars($produk['nama']) . '" class="img-fluid" 
